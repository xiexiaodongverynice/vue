...
$.extend({
	...
	wordTokenEditer: function (paramsObj) {
		...
		var editBaseAttr = {
			contenteditable: paramsObj.editable
		};
			...
			if (anchorNode && sctnt) {
				...
				if (slen <= nlen && pnode && pname == "DIV" && nname == "#text" && slen > 0) {
					...
					if (sctnt.length > 0) {
						...
						$(snode).addClass("word-token-editer-a word-token-editer-def spanstylecss");
		...
		function nodeStateClick(node) {
			...
			$(divlist).children(".word-token-editer-a-click").removeClass("word-token-editer-a-click");
