<template>
  <section v-loading="loading" class="cms-body">
    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="checkIds">
      ...
      <el-table-column prop="cjycjl" label="常见异常结论" align="center"></el-table-column>
      ...
      <el-table-column prop="name" label="操作" width="90" align="center">
        <div slot-scope="scope">
          <!-- <cms-button  type="edit" @click.native="routerLink('/jianCha/edit','edit',scope.row.pid)"
                        v-perms="'/jianCha/edit'"
                    ></cms-button> -->
          <cms-button type="huaci" @click.native="routerLink('/jianCha/oprate','oprate',scope.row.pid)" v-perms="'/jianCha/oprate'"></cms-button>
          <!-- <cms-button type="view" @click.native="routerLink('/jianCha/view','view',scope.row.pid)" v-perms="'/jianCha/view'"></cms-button> -->
        </div>
      </el-table-column>
...
export default {
  ...
  methods: {
    jbQuery() {
      ...
      sessionStorage.setItem("jcListCanShu", JSON.stringify(this.canShu));
      this.query();
    }
  },
  created() {
    sessionStorage.setItem("jcListCanShu", "");
    this.initTableData(this.$api.jianChaList, this.params);
  }
};
</script>
