  ...
  <section class="cms-body zsjc-oprate" v-loading="loading">
    ...
    <div class="main">
      ...
      <div class="art-title">
        ...
        <div>{{dataInfo.xmmc}}</div>
         <div class="fu-biao-ti">
          <span v-if="dataInfo.cjycjl">常见异常结论：{{dataInfo.cjycjl}}</span>
        </div>
        <div class="fu-biao-ti">
...
export default {
  ...
  data() {
    return {
      beanId: "zsjcJianChaJieGuoService",
      dialogVisible: false,
      prePid: null,
      nextPid: null
    };
  ...
  methods: {
    ...
    getDataInfo(id) {
      ...
      let listCanShu = sessionStorage.getItem("jcListCanShu");
      let canShu = {};
      ...
      axios
        .all([
          axios.post(api.jianChaDetail, paras) //axios批量发送请求
        ])
        .then(
          axios.spread(data => {
            this.loading = false;
