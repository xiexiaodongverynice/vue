<template>
  <section v-loading="loading" class="cms-body">
    <div class="cms-list-header flex-between">
      <div>
        <cms-input label="疾病名称" v-model="canShu.name"></cms-input>
    ...
    <div class="cms-list-header flex-between">
      ...
      <!-- 右操作 -->
      <div>
        <el-button type="primary" icon="el-icon-upload" @click="updateEsData('esDataJiBingService')">更新搜索引擎数据</el-button>
    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="checkIds">
      ...
      <el-table-column prop="name" label="操作" width="90" align="center">
        <div slot-scope="scope">
          <!-- <cms-button  type="edit" @click.native="routerLink('/jiBing/edit','edit',scope.row.pid)"
                        v-perms="'/jiBing/edit'"
                    ></cms-button> -->
          <cms-button type="huaci" @click.native="routerLink('/jiBing/oprate','oprate',scope.row.pid)" v-perms="'/jiBing/oprate'"></cms-button>
          <!-- <cms-button type="view" @click.native="routerLink('/jiBing/view','view',scope.row.pid)" v-perms="'/jiBing/view'"></cms-button> -->
        </div>
      </el-table-column>
...
export default {
  ...
  methods: {
    /**
     * 查询列表数据，并把查询条件存入session
     */
    jbQuery() {
      this.canShu.pageNo=null;
      sessionStorage.setItem("jbListCanShu", JSON.stringify(this.canShu));
      this.query();
    }
    
  },
  created() {
    sessionStorage.setItem("jbListCanShu", "");
