  ...
  <section class="cms-body zsjc-oprate" v-loading="loading">
    ...
    <div class="main">
      ...
      <div class="art-title">
        ...
        <div style="margin:0 70px;">{{dataInfo.xmmc}}</div>
...
export default {
  ...
  data() {
    return {
      beanId: "zsjcFuYouJianKangService",
      dialogVisible: false,
      prePid: null,
      nextPid: null
  ...
  methods: {
    ...
    getDataInfo(id) {
      ...
      let listCanShu = sessionStorage.getItem("fyListCanShu");
      let canShu = {};
      ...
      axios
        .all([
          axios.post(api.fyjkDetail, paras) //axios批量发送请求
        ])
        .then(
          axios.spread(data => {
            this.loading = false;
