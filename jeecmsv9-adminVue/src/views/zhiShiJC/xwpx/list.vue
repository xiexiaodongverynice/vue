<template>
  <section v-loading="loading" class="cms-body">
    <div class="cms-list-header flex-between">
      <div>
        <cms-input label="适应症" v-model="canShu.name"></cms-input>
    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="checkIds">
      <el-table-column prop="shiyingzhengName" label="适应症" align="center"></el-table-column>
      <el-table-column prop="zuheXueNames" label="组合穴名称" align="center"></el-table-column>
      ...
      <el-table-column prop="name" label="操作" width="90" align="center">
        <div slot-scope="scope">
          <!-- <cms-button  type="edit" @click.native="routerLink('/xwpx/edit','edit',scope.row.pid)"
                        v-perms="'/xwpx/edit'"
                    ></cms-button> -->
          <cms-button type="huaci" @click.native="routerLink('/xwpx/oprate','oprate',scope.row.pid)" v-perms="'/xwpx/oprate'"></cms-button>
          <!-- <cms-button type="view" @click.native="routerLink('/xwpx/view','view',scope.row.pid)" v-perms="'/xwpx/view'"></cms-button> -->
        </div>
      </el-table-column>
...
export default {
  ...
  methods: {
    jbQuery() {
      ...
      sessionStorage.setItem("xwListCanShu", JSON.stringify(this.canShu));
      this.query();
    }
  },
  created() {
    sessionStorage.setItem("xwListCanShu", "");
    this.initTableData(this.$api.xwpxList, this.params);
  }
};
</script>
