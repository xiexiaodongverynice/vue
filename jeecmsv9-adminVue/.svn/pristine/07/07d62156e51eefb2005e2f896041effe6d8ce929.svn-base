  ...
  <section class="cms-body zsjc-oprate" v-loading="loading">
    ...
    <div class="main">
      ...
      <div class="art-title">
        ...
        <div>{{dataInfo.pYpmc}}</div>
        <div class="fu-biao-ti">
          <span v-if="dataInfo.pCj">厂家：{{dataInfo.pCj}}</span>
...
export default {
  ...
  methods: {
    ...
    getDataInfo(id) {
      ...
      let listCanShu = sessionStorage.getItem("ypListCanShu");
      let canShu = {};
      ...
      axios
        .all([
          axios.post(api.yaoPinDetail, paras) //axios批量发送请求
        ])
        .then(
          axios.spread(data => {
            this.loading = false;
