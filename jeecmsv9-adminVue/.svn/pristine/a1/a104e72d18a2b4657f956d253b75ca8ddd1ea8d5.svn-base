  ...
  <section class="cms-body zsjc-oprate" v-loading="loading">
    ...
    <div class="main">
      ...
      <div class="art-title">
        ...
        <div>{{dataInfo.jyXmMc}}</div>
        <div class="fu-biao-ti">
          <span v-if="dataInfo.shiYongTj">适用条件：{{dataInfo.shiYongTj}}</span>
          <span v-if="dataInfo.biaoZhun">标准：{{dataInfo.biaoZhun}}</span>
          <span v-if="dataInfo.biaoZhun">分类：{{dataInfo.yiJiFenLei}}>{{dataInfo.erJiFenLei}}</span>
        </div>
        <div class="fu-biao-ti">
      ...
      <div class="art-body" v-if="dataInfo.disList&&dataInfo.disList.length>0">
        <!-- 操作控制面板 -->
      ...
      <div v-if="!dataInfo.disList||dataInfo.disList.length==0" class="nullTip">暂无相关内容！</div>
...
export default {
  ...
  data() {
    return {
      beanId: "zsjcJianYanZhiBiaoService",
      dialogVisible: false,
      prePid: null,
      nextPid: null
    };
  ...
  methods: {
    ...
    getDataInfo(id) {
      ...
      let listCanShu = sessionStorage.getItem("jyListCanShu");
      let canShu = {};
      ...
      axios
        .all([
          axios.post(api.jianYanDetail, paras) //axios批量发送请求
        ])
        .then(
          axios.spread(data => {
            this.loading = false;
