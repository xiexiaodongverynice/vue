        ...
        <el-form  ref="form" :model="info" :rules="rules" class="cms-form" label-width="162px" v-loading="loading" v-if="restState">        
           ...
           <!-- 系统字段 -->
              <el-form-item v-for="(item,index) in itemList" :key="index"             
                ...
                v-if="item.field!='channelId'&&item.field!='title'&&item.field!='typeImg'" 
              >  
              <!-- 文本 -->
             ...
             <!-- 标题，内容图片 类型图 -->
                <cms-upload :src="info[item.field]" :field="item.field" v-if="item.field=='titleImg'"  :isMark="false" 
                  @get="getFieldImg"> </cms-upload>
                <cms-upload :src="info[item.field]" :field="item.field" v-if="item.field=='contentImg'"  :isMark="false"
                @get="getFieldImg"> </cms-upload>
          <!-- 附件 -->
              ...
              <!-- 固顶级别 -->
                <div v-if="item.field=='topLevel'">
                  <el-input class="w50" type="number" v-model="info['topLevel']"></el-input>
                   <label  class="ml-24">固顶到期时间</label>
...
export default {
  ...
  methods: {
    ...
    getEditorContent() {
      ...
      if (this.info.tx1 != undefined) {
        this.info.txt1 = this.ue[1].getContent();
      }
      if (this.info.tx2 != undefined) {
