export default {
    ...
    methods:{
        ...
        getTableData(params) {//获取表格数据   
            ...
            this.$http
                ...
                .then(res => {
                    this.loading = false;
                    if(res.code=='200'){
                        if (res.body.zongtiaoshu!=undefined) {
                            this.total = res.body.zongtiaoshu;
                        }
                        this.tableData = res.body.zbList;
                        this.state=true;
        ...
        getPages(pageNo, pageSize) {
            //获取翻页数据
            this.params.num = pageNo;
            this.params.xsts = pageSize;
        ...
        deleteBatch(url,ids) { //删除
            this.$confirm('是否确定删除？', '警告', { type: "error" })
                .then(mes => {
                    this.$http.post(url, { id: ids,user:localStorage.getItem('userName') }).then(res => {
                        if (res.code == "200") {
