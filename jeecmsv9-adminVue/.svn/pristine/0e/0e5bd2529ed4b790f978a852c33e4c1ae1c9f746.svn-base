<template>
  <section v-loading="loading" class="cms-body">
    ...
    <div class="cms-list-header flex-between">
      ...
      <!-- 右操作 -->
      <div>
        <el-button type="primary" icon="el-icon-upload" @click="updateEsData('esDataFuYouJianKangService')">更新搜索引擎数据</el-button>
    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="checkIds">
      ...
      <el-table-column prop="name" label="操作" width="90" align="center">
        <div slot-scope="scope">
          <!-- <cms-button  type="edit" @click.native="routerLink('/fyjk/edit','edit',scope.row.pid)"
                        v-perms="'/fyjk/edit'"
                    ></cms-button> -->
          <cms-button type="huaci" @click.native="routerLink('/fyjk/oprate','oprate',scope.row.pid)" v-perms="'/fyjk/oprate'"></cms-button>
          <!-- <cms-button type="view" @click.native="routerLink('/fyjk/view','view',scope.row.pid)" v-perms="'/fyjk/view'"></cms-button> -->
        </div>
      </el-table-column>
...
export default {
  ...
  methods: {
    jbQuery() {
      ...
      sessionStorage.setItem("fyListCanShu", JSON.stringify(this.canShu));
      this.query();
    }
  },
  created() {
    sessionStorage.setItem("fyListCanShu", "");
    this.initTableData(this.$api.fyjkList, this.params);
  }
};
</script>
